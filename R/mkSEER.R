mkSEER<-function(df,seerHome="~/data/SEER",dataset=c("00","73","92"), SQL = TRUE){
#	if (!require(LaF)) print("R Package LaF must be installed")
  ...
  seerHome=path.expand(seerHome)
	...
	f=switch(dataset,
	         "00" = file.path(seerHome,"populations/expanded.race.by.hispanic/yr2000_2011.ca_ky_lo_nj_ga/19agegroups.txt"),
	         "73" = file.path(seerHome,"populations/white_black_other/yr1973_2011.seer9/19agegroups.txt"),
	         "92" = file.path(seerHome,"populations/expanded.race.by.hispanic/yr1992_2011.seer9.plus.sj_la_rg_ak/19agegroups.txt"))
	ptm <- proc.time()
  ...
  # comment this chunk out if it takes too much time
	s=switch(dataset,
