...
for (k in c(8))
{
	...
	DF=DF[!is.na(DF$ICD10),]  # get rid of missing ICD10 entries
	if (k==8) { canc=c( "Hodgkins","MML" ,"ALL", "CLL", "AML", "CML")
		code=c(        "C810",    "C900","C910","C911","C920","C921")}
	for (j in 1:length(code))
	{	
		windows(width=12,height=5,xpos=-150)
		par(mfrow=c(1,3),mar=c(4.7,0,2,0),oma=c(0,7.1,4,0),lwd=3,cex.lab=2.8,cex.axis=2.5,cex.main=2.8)
		Indx=(DF$ICD10==code[j])&(DF$numprims==1) 
